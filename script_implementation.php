// Open shared memory object written from readmqttbms.php
...
// Create BMS feeback command for inverter
if($bms){
	$bmscmd = shmop_read($sh_bms, 0, 56);
	$bmscrc = paddings(genCRC($bmscmd),4);
	$bmscrc1 = substr($bmscrc,-4,2);
	$bmscrc2 = substr($bmscrc,-2);
	fwrite($fp,$bmscmd.chr(hexdec($bmscrc1)).chr(hexdec($bmscrc2)).chr(0x0d)); // send D054BMS string with CRC to inverter, here with open handle $fp
}
